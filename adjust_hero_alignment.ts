...
if (startIndex !== -1 && endIndex !== -1) {
              ...
              {mainArticle && (
                ...
                >
                  ...
                  <div className={\`absolute inset-0 \${mainArticle.image_url ? 'bg-gradient-to-t from-primary-navy/90 via-transparent to-transparent' : ''} flex flex-col justify-end p-8 md:p-10 pb-16 md:pb-20\`}>
                    ...
                    <div className={\`flex flex-col gap-3 w-full max-w-4xl mx-auto \${mainArticle.category_slug === 'short-urgent' ? 'items-start text-left ml-0' : 'items-end text-right'}\`}>
                      ...
                      <motion.div
                        ...
                        className="mb-2"
                      >
                         <span className={\`bg-primary-crimson text-white border border-primary-crimson \${mainArticle.category_slug === 'short-urgent' ? 'px-3 py-1 text-[10px] md:text-sm' : 'px-4 py-1.5 text-xs md:text-lg'} font-black uppercase tracking-[0.2em] rounded-full shadow-[0_0_20px_rgba(225,29,72,0.5)] animate-pulse\`}>
                           عاجل
                         </span>
                      </motion.div>
    ...
    console.log('Successfully adjusted Short Urgent alignment to the left');
} else {
    console.error('Could not find hero markers in App.tsx');
}
