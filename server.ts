      ...
      if (email) {
        try {
          ...
          await transporter.sendMail({
            ...
            to: email,
            subject: 'تفاصيل حسابك الجديد في لوحة التحكم',
            html: `
              <div dir="rtl" style="font-family: sans-serif; line-height: 1.6; color: #333;">
                ...
                <div style="background: #f4f6f8; padding: 20px; border-radius: 8px; margin: 20px 0;">
                  ...
                  <p><strong>كلمة المرور ${isAutoGenerated ? '(المولدة آلياً)' : ''}:</strong> <code style="background:#e2e8f0;padding:2px 6px;border-radius:4px;font-size:18px;">${password}</code></p>
                </div>
                <p style="color: #666; font-size: 14px;">يرجى تسجيل الدخول وتغيير كلمة المرور الخاصة بك في أقرب وقت لحماية حسابك.</p>
